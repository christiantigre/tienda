	...
	*/
	if ($_GET["firmarPDF"] == true) {
		try {
			...
			$filename = "resource/Loremipsum.pdf";
			$file = fopen($filename, 'r');
			$datos2 = fread($file, filesize($filename));
			fclose($file);

			//Subimos el documento al servidor
			$idFirmas=$viafirmaClient->prepareFirmaWithTypeFileAndFormatSign("exampleSign.pdf",MimeType::$PDF,$datos,SignatureType::$PDF_PKCS7);
			$idFirmas.= ";".$viafirmaClient->prepareFirmaWithTypeFileAndFormatSign("Loremipsum.pdf",MimeType::$PDF,$datos2,SignatureType::$PDF_PKCS7);
			
			//Iniciamos el proceso de firma
			//Url de retorno a la aplicación (tras el proceso de firma)
			$viafirmaClient->solicitarFirmasIndependientes($idFirmas,$VIAFIRMA_RETURN_TO_URL);
					...
					<div class="col width-43 append-02">
						<div class="box">
							...
							<div class="box-content">
								<p>En este ejemplo se realiza la firma de varios documentos PDF en un solo proceso con formato PDF Signature en Viafirma con la intervención directa del usuario.</p>
