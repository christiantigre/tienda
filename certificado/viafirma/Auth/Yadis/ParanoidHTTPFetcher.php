 ...
 * Interface import
 */
require_once "viafirma/Auth/Yadis/HTTPFetcher.php";
...
class Auth_Yadis_ParanoidHTTPFetcher extends Auth_Yadis_HTTPFetcher {
    ...
    function get($url, $extra_headers = null)
    {
        ...
        while ($redir && ($off > 0)) {
            ...
            // Add support by proxy
            global $proxyHost,$proxyPort,$proxyUser,$proxyPassword;
            //PROXY
			if($proxyHost != "" && $proxyPort != ""){
				curl_setopt($c, CURLOPT_PROXY, $proxyHost);
				curl_setopt($c, CURLOPT_PROXYPORT, $proxyPort);
				if($proxyUser != ""){
					curl_setopt($c,CURLOPT_PROXYUSERPWD,"$proxyUser:$proxyPassword");
				}
			}
